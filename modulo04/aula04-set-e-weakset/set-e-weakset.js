...
// no Arrray comum, para se um item existe
...
// na documentação tem exemplos sobre como fazer uma interceção, saber o que tem em uma lista e nãp
// tem na outr e assim por diante
...
assert.deepStrictEqual(Array.from(intersection), ['jhonatan'])

const difference = new Set([...user01].filter(user => !user02.has(user)))
assert.deepStrictEqual(Array.from(difference), ['ronaldo', 'messi'])


// weakSet

// mesma ideia do WeakMap
// não é enumerável (interável)
